...
// - When stay there without interactions, LED on, white light,  but with a random blink at random time, for example, every random(5,10) seconds, it blinks random(1,5) times
...
unsigned long nextBlinkInterval = random(5000, 10000);  //random interval for blink
...
void loop() {
  ...
  if (tiltState == LOW && lightValue <= 1025) {
    setLED(255, 255, 255);
    ...
    if (currentTime - lastBlinkTime > nextBlinkInterval) {
      ...
      nextBlinkInterval = random(5000, 10000);  // Set next random interval
    }
  }
  ...
  if (lightValue > 1025 && (currentTime - lastLightTime > lightCooldown)) {
    Serial.println(lightValue);
